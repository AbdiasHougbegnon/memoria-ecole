...
- **Le même patron `@Transactional` manquant pourrait exister ailleurs** dans le projet partout où une requête dérivée de suppression/modification est appelée en dehors d'une transaction explicite — seul le cas `MembreCouloirRepository` a été réellement observé et corrigé ici ; pas de passe générale de vérification sur le reste du code.

## 8. Pour reprendre seul

- Code de référence exact : `git checkout phase-5-gestion-couloir`
- Si le même bug `@Transactional` doit être vérifié ailleurs : chercher tout appel à une méthode de repository **définie dans l'interface** (pas héritée de `JpaRepository`) qui modifie des données (`deleteBy...`, `updateBy...`), et confirmer qu'elle est appelée depuis une méthode de service annotée `@Transactional`.
